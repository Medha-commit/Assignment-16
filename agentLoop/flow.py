...
class AgentLoop4:
    ...
    async def _execute_step(self, step_id, context):
        ...
        if result["success"]:
            ...
            # Check for call_self - Support up to 4 iterations
            if output.get("call_self"):
                print(f"🔄 {agent_type} (Step {step_id}): Starting call_self iterations...")
                # Initialize iterations data
                iterations_data = [{"iteration": 1, "output": output}]
                current_output = output
                current_inputs = inputs
                final_result = result
                
                # Execute up to 3 additional iterations (total of 4)
                for iteration_num in range(2, 5):  # iterations 2, 3, 4
                    print(f"🔄 {agent_type} (Step {step_id}): Executing iteration {iteration_num}...")
                    
                    # Handle code execution if needed
                    if context._has_executable_code(current_output):
                        print(f"  ⚙️ Executing code from iteration {iteration_num-1}...")
                        ...
                        if execution_result.get("status") == "success":
                            ...
                            current_inputs = {**current_inputs, **execution_data}
                            print(f"  ✅ Code execution successful for iteration {iteration_num-1}")
                        else:
                            print(f"  ❌ Code execution failed for iteration {iteration_num-1}")
                    
                    # Check if we should continue to next iteration
                    if not current_output.get("call_self"):
                        print(f"🛑 {agent_type} (Step {step_id}): call_self set to false, stopping at iteration {iteration_num-1}")
                        break
                    ...
                    if next_result["success"]:
                        iterations_data.append({"iteration": iteration_num, "output": next_result["output"]})
                        ...
                        print(f"  ❌ Iteration {iteration_num} failed: {next_result.get('error', 'Unknown error')}")
                        break
                ...
                print(f"🎉 {agent_type} (Step {step_id}): Completed {len(iterations_data)} iterations")
                return final_result
            else:
                return result
        else:
